...
# MÓDULO PRINCIPAL
print('main_BiTCN.py iniciado')

# Importar módulos e dataframes necessários
from configuracoes.imports import *
from configuracoes.configuracoes_BiTCN import marca, gerar_combinacoes_parametros
...
# Carregar o módulo do modelo BiTCN dinamicamente
modulo_modelo = importlib.import_module('models.model_BiTCN')

# Ajustar para chamar a função correta, no caso do BiTCN
funcao_treinar = getattr(modulo_modelo, 'treinar_BiTCN')  # Função específica para o BiTCN

# Gerar as combinações de parâmetros para o BiTCN
param_combinations = gerar_combinacoes_parametros('BiTCN')

# Inicializar DataFrame de métricas para armazenar todas as combinações
metricas_df_final = pd.DataFrame(columns=['max_steps', 'learning_rate', 'batch_size', 'MAE', 'MSE', 'RMSE'])

# Iterar sobre as combinações de parâmetros
for params in param_combinations:
    max_steps, learning_rate, batch_size = params
    col_name = f'BiTCN_steps{max_steps}_lr{learning_rate}_batch{batch_size}'
    print(f'Testando parâmetros: max_steps={max_steps}, learning_rate={learning_rate}, batch_size={batch_size}')
    ...
    if data_neural_hat is not None:
        ...
        data_neural_hat.rename(columns={'BiTCN': col_name}, inplace=True)
...
if not metricas_df_final.empty:
    csv_file_path = f'{output_dir}/forecast_with_metrics_BiTCN_{horario_atual}.csv'
...
if 'y' in data_neural_hat_final_plot.columns:
    ...
    plot_file_path = f'{output_dir}/plot_image_BiTCN_{horario_atual}.png'
...
print('main_BiTCN.py finalizado')