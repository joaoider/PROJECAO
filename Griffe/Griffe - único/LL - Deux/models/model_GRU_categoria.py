print('model_GRU_categoria.py iniciado')

from configuracoes_modelos.imports import *
from LL_base_categoria import data_neural_train, data_neural_test, static_df
...
def treinar_GRU(max_steps, learning_rate, batch_size, encoder_hidden_size, decoder_hidden_size, 
    ...
    nf.fit(df = data_neural_train, static_df = static_df)
    ...
    missing_combinations = nf.get_missing_future(futr_df = data_neural_test)
    if not missing_combinations.empty:
      print(f"Missing combinations found:\n{missing_combinations}")

    # Gerar previsões
    data_neural_hat = nf.predict(futr_df = data_neural_test)

    print('treinar_GRU_categoria finalizado')
